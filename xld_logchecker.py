...
def sha256(data, initial_state):
    ...
    for start in range(0, len(data), 64):
        ...
        round_state = 4 * [
            int.from_bytes(chunk[i : i + 4], "big") for i in range(0, len(chunk), 4)
        ]

        for i in range(16, 64):
            s0 = (
                rotate_right(round_state[i - 15], 7)
                ^ rotate_right(round_state[i - 15], 18)
                ^ (round_state[i - 15] >> 3)
            )
            s1 = (
                rotate_right(round_state[i - 2], 17)
                ^ rotate_right(round_state[i - 2], 19)
                ^ (round_state[i - 2] >> 10)
            )

            round_state[i] = (
                round_state[i - 16] + s0 + round_state[i - 7] + s1
            ) & 0xFFFFFFFF
...
def xld_verify(data):
    ...
    # No padding bytes
    signature = nonstandard_base64_encode(
        scrambled, "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz._"
    ).rstrip("=")
